...
# update DNS
#echo 'nameserver 10.25.0.10' > /etc/resolv.conf
#echo 'search nifi.nifi.svc.cluster.local nifi.svc.cluster.local svc.cluster.local cluster.local aur.test.devops.com aur.devops.com devops.com' >> /etc/resolv.conf
#echo 'options ndots:5' >> /etc/resolv.conf

path=/opt/nifi/nifi-1.7.0

## ------------------------
# move to persistent storage
sed -i -e "s|^nifi.authorizer.configuration.file=.*$|nifi.authorizer.configuration.file=/mnt/conf/authorizers.xml|" $path/conf/nifi.properties
sed -i -e "s|^nifi.flow.configuration.file=.*$|nifi.flow.configuration.file=/mnt/conf/flow.xml.gz|" $path/conf/nifi.properties
...
sed -i -e "s|\./state/local|/mnt/state/local|" $path/conf/state-management.xml
# ----------------------
...
# ------------------------
echo "####### Enabling JMX and Prometheus metrics setup #######" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
echo "java.arg.101=-Dcom.sun.management.jmxremote.local.only=true" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
echo "java.arg.102=-Dcom.sun.management.jmxremote" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
echo "java.arg.103=-Dcom.sun.management.jmxremote.authenticate=false" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
echo "java.arg.104=-Dcom.sun.management.jmxremote.ssl=false" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
echo "java.arg.104=-Dcom.sun.management.jmxremote.port=30007" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
echo "java.arg.105=-javaagent:/opt/nifi/nifi-1.7.0/lib/jmx_prometheus_javaagent-0.3.1.jar=8079:/tmp/prometheus_jmx_exporter.yml" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
echo "####### Enabling JMX and Prometheus metrics setup #######" >> /opt/nifi/nifi-1.7.0/conf/bootstrap.conf
# ------------------------
